...
function Box(props: any) {
  ...
  const [matchesLookup, setMatchesLookup] = useState({});
  ...
  return (
    <>
      <div className="shadowy" onClick={clickHandlerExpander}>
        {/* <h1 className="d-flex justify-content-between">
          <span className=''>{props.name}</span>
          <span className=''>{props.data.summonerLevel}</span></h1> */}
        {
          isExpanded
            ?
            <Col>
            ...
            //             {matchesLookup[x] ? matchesLookup[x].participants.filter(x => x.summonerId === props.data.id)[0].win ? 'W' : '' : ''}
